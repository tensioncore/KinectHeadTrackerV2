...
void KinectSensor::Release()
{
    ...
    // Stream handles are owned by Nui; set null for safety
    m_pDepthStreamHandle = NULL;
    m_pVideoStreamHandle = NULL;
...
DWORD WINAPI KinectSensor::ProcessThread(LPVOID pParam)
{
    ...
    while (true)
    {
        DWORD wait = WaitForMultipleObjects(sizeof(hEvents) / sizeof(hEvents[0]), hEvents, FALSE, 100);
        ...
        // Process signaled events (manual-reset events MUST be reset)
        ...
        {
            pthis->GotVideoAlert();
            ResetEvent(pthis->m_hNextVideoFrameEvent);
        }
        else if (wait == WAIT_OBJECT_0 + 3)
        {
            pthis->GotSkeletonAlert();
            pthis->m_SkeletonTotal++;
            ResetEvent(pthis->m_hNextSkeletonEvent);
        }
    }
...
void KinectSensor::GotVideoAlert()
{
    ...
    if (LockedRect.Pitch && LockedRect.pBits)
    {   // Copy video frame to face tracking
        memcpy(m_VideoBuffer->GetBuffer(), PBYTE(LockedRect.pBits),
            min(m_VideoBuffer->GetBufferSize(), UINT(pTexture->BufferLen())));
    }
    else
    {
    ...
    // CRITICAL: UnlockRect or you leak/lock resources and eventually die
...
void KinectSensor::GotDepthAlert()
{
    ...
    {   // Copy depth frame to face tracking
        memcpy(m_DepthBuffer->GetBuffer(), PBYTE(LockedRect.pBits),
            min(m_DepthBuffer->GetBufferSize(), UINT(pTexture->BufferLen())));
    }
    else
    {
